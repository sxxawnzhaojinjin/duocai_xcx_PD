...
Page({
  ...
  itemClick(e) {
    ...
    wx.navigateTo({
      url: './warehouseDetail?itemId=' + itemId + "&itemName=" + itemName + "&itemNo=" + itemNo + "&pictureUrl=" + pictureUrl + "&index=" + index,
    })
  },
    ...
    getSelectProduct(param).then(res => {
        ...
        if (res.code == 200) {
          ...
          for (let j in list) {
            ...
            let itemId = list[j].itemId;

            let a = this.data.newList
            let index = a.findIndex(item => item.itemId == itemId);

            if (index > -1) {
              list[j].falg = true;
              list[j].selectNumber = a[index].selectNumber
              list[j].hasSelectSkus = a[index].hasSelectSkus
            }
          }
  ...
  cancel(e) {
    ...
    let a = this.data.newList
    let index5 = a.findIndex(item => item.itemId == itemId);
    if (index5 > -1) {
      a.splice(index5, 1);
    }
    this.setData({
  ...
  submit() {
    ...
    for (let i in newList) {
      let skuList = newList[i].hasSelectSkus
      for (let j in skuList) {
        sku.push(skuList[j])
      }
    }
    ...
    savePurchaseBill({
      ...
      .then(data => {
        wx.navigateTo({
          url: './warehouseEntryDetail?entryId=' + data.data,
        })
        that.setData({
          pageNum: 1
        }, () => {
